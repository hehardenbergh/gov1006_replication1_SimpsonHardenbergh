...
# "Republican" and when it = 0, "Democrat". Our best attempt is the following:
      # table_1 %>% 
      #   select(all_judges$republican) %>% 
      #   mutate(republican, case_when(republican == `0` ~ "Democrat",
...
# and now for the table: use kable() instead of xtable() to sidestep the latex
# formatting.
...
aa %>% 
  ...
  # group_rows("Number of Children", 1, 3) %>% 
  # group_rows("Number of Daughters", 4, 6)
...
aa <- table(na.omit(all_judges$child))
...
```{r}
#class notes
x <- as_tibble(judge.means)
#lm.obj <- lm(lib_vote_share ~ I(girls > 0) + as.factor(child), data = x, weights = no_cases)
#lm.obj <- lm(lib_vote_share ~ as.factor(child), data = x, weights = no_cases)
lm.obj <- lm(lib_vote_share ~ I(girls > 0) + as.factor(circuit.1), data = x, weights = no_cases)
summary(lm.obj)
```
...
=======
          # my.out11, 
          # my.out12, 
>>>>>>> 015140744895bcee2feb94907c8d17f6896a0f88
          style = "ajps",
          omit.stat = c("f","ser"),
