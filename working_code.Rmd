...
table_1 <- combine(aa, bb)

# and now for the table: use kable() instead of xtable() to sidestep the latex
# formatting.

kable(table_1,
      caption = "TABLE 1 Number of Children and Girls for U.S. Courts of Appeals Judges Participating in Gender-Related Cases, 1996-2002", digits = 1) %>% 
...
mean.birthyear <- cbind(mean(na.omit(all$yearb)), 
...
# a row that shows the number of judges for each column we will replicate: All,
# Democrats, Republicans, Women, and Men. 

no_judges <- cbind(nrow(all),
                   nrow(dems),
                   nrow(reps),
                   nrow(women),
                   nrow(men))
