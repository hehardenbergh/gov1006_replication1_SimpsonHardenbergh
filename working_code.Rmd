...
Excellent to have an intro

# Data Introduction

This paper uses two datasets: data on 244 individual judges and 4,377 cases. The dataset on judges contains identification and demographic information (the judge’s name, circuit number, gender, age, year of birth, race, religion, and the party of the appointing president), information on the judge's children (the total number of children, the number of daughters, the number of sons), and the proportion of gender-related cases in which the judge votes progressively. The dataset on cases includes information on the cases themselves, the area of law they are in, the judges who decided them, and whether or not a progressive vote occurred.
...
# The Zelig function estimates statistical models for data frames, based on
# which model you want to estimate with.

sim <- Zelig:::sim

# always useful to have comments like this
...
# This loads three data sets 1. Gender cases 2. Cleaned by-judge data 3.
# Non-gender cases, downloaded from Kuersten and Haire's website (Commented
# out; their data are available from their website)
...
# Authors Glynn and Sen used a dataset and code from Ashlyn Kuersten/Susan
# Haire's website to create a non-gender cases dataframe. Once downloaded, call:
# cases <- read.csv("cases.for.analysis.csv"). since we don't need that data for
# this replication, we did not grab that extra data.

judge.means <- read.csv("glynn_sen_daughters_by_judge.csv", 
                        stringsAsFactors = FALSE)

# Subset judge data to those for whom we have fertility data

all_judges <- subset(judge.means, 
              girls != "NaN") 
```

```{r, results= 'asis'}
## Table 1 Number of kids and daughters

# very detailed comments

# create objects for number of children and number of girls per judge, per
# political party. As of now, "bb" is only producing a table that shows judges
# having up to 5 children. Additionally, we need to recode the republican
# variable as a character so that, when the value = 1, we want it to say
# "Republican" and when it = 0, "Democrat". Our best attempt is the following:
#           sub_table1 <- c("republican",
#                          "child",
#                           "girls")
#           table1 <- all_judges[sub_table1] %>% 
#             mutate(republican, case_when(republican %in% 0 ~ "Democrat",
#                                         republican %in% 1 ~ "Republican"))

# Then, ideally, you would have one unified table, where you could then use kable() and the kableExtra package to group the rows by "Number of Children" and "Number of Girls." Instead, the authors use base r to combine two separate tables into one by factors of the variable "republican", which works, but is less 'pretty' and more of a hassle. I think there is an easier way.

aa <- table(all_judges$republican, all_judges$child)

# something happened when they made this "bb" table: only 7 columns print instead of
# 10.

bb <- table(all_judges$republican, all_judges$girls)

rownames(aa) <- c("Democrat",
                  "Republican")

aa %>%
  kable(caption = "TABLE 1 Number of Children and Girls for U.S. Courts of Appeals Judges Participating in Gender-Related Cases, 1996-2002",
        format = "latex") 


rownames(bb) <- c("Democrat",
                  "Republican")

bb %>%
  kable(format = "latex")

# Definitely want to see some more labels on Table 1 and clean it up a bit/keep it all together
...
# Make objects for each subset of data, to create mean values of variables next.
# Make objects to designate judges' political parties and for their genders.

dems <- subset(all_judges, 
               republican == 0)
reps <- subset(all_judges, 
               republican == 1)
women <- subset(all_judges,
                woman == 1)
men <- subset(all_judges, 
              woman == 0)

# call mean for number of kids, excluding NAs with na.omit().

mean.kids <-  cbind(mean(na.omit(all_judges$child)), 
...
# mean number of girls, same with the na.omit() function.

mean.girls <- cbind(mean(na.omit(all_judges$girls)), 
...
# count proportion of judges who have 0 kids. Omit rows with missing values
# using na.omit() for each judge variable: their # kids, # daughters, their
# political party and their gender.

prop.zero <- cbind(prop.table(table(na.omit(all_judges$child)))[1],
...
# count proportion of judges who have 1 kid. Omit rows with missing values
# using na.omit()

prop.one <- cbind(prop.table(table(na.omit(all_judges$child)))[2],
...
# count proportion of judges who have 2 kids. Omit rows with missing values
# using na.omit()

prop.two <- cbind(prop.table(table(na.omit(all_judges$child)))[3],
...
# count proportion of judges who have 3 kids. Omit rows with missing values
# using na.omit()

prop.three <- cbind(prop.table(table(na.omit(all_judges$child)))[4],
...
# count proportion of judges who have 4 kids. Omit rows with missing values
# using na.omit()

prop.four <- cbind(prop.table(table(na.omit(all_judges$child)))[5],
...
# count proportion of judges who have 5 kids. Omit rows with missing values
# using na.omit()

prop.five <- cbind(prop.table(table(na.omit(all_judges$child)))[6],
...
# Sum up number of children per variable in preparation for making variables
# that show proportion of judges with 0, 1, 2, 3, 4, 5, and 6 kids.

aa2 <- table(na.omit(all_judges$child))
...
# make a variable for judges who have 6 or more kids
	
prop.six.or.greater <- cbind(plus6.all, 
                             plus6.dems, 
                             plus6.reps, 
                             plus6.women, 
                             plus6.men)
	
# find proportion female 

mean.female <- cbind(mean(na.omit(all_judges$woman)), 
...
mean.rep <- cbind(mean(na.omit(all_judges$republican)), 
...
# find proportion white

mean.white <- cbind(mean(na.omit(all_judges$race == 1)), 
...
# find mean birth year for judges

mean.birthyear <- cbind(mean(na.omit(all_judges$yearb)), 
...
no_judges <- cbind(nrow(all_judges),
                   nrow(dems),
                   nrow(reps),
                   nrow(women),
                   nrow(men))
...
colnames(demographic_table) <- c("All", 
                                 "Democrats",
                                 "Republicans",
                                 "Women",
                                 "Men")
...
rownames(demographic_table) <- c("Mean No. Children",
                                 ...
                                 "Mean Year Born", 
                                 "N")
# There are a total of 224 judges with fertility data. We need to combine the
# following buckets: all judges, democrats, republicans, women, and men to make
# a row that shows the number of judges for each column we will replicate: all_judges,
# Democrats, Republicans, Women, and Men.
...
nice work on table 2!

```{r, results='asis'}
## Calculating the Weights (Number of Cases)

no_cases <- matrix(data = NA,
                   nrow = nrow(judge.means),
                   ncol = 1)
...
# comments on what this is doing would be useful since this is an important outcome

no_liberalvote <- matrix(data = NA,
                         nrow = nrow(judge.means),
                         ncol = 1)

for(i in 1:length(no_liberalvote)){
	stuff <- women.cases[which(women.cases$name == judge.means$name[i]),]
	no_liberalvote[i] <- nrow(subset(stuff,
	                                 vote == 2 | vote == 3))}

lib_vote_share <- no_liberalvote/no_cases

judge.means <- cbind(judge.means,
                     no_liberalvote,
                     lib_vote_share)
judge.means <- subset(judge.means,
                      girls != "NaN")
...
## just women:
women.means <- subset(judge.means,
                      woman == 1)

## just men:
men.means <- subset(judge.means, 
                    woman == 0)

## just republicans:
rep.means <- subset(judge.means, 
                    republican == 1)

## just democrats
dem.means <- subset(judge.means, 
                    republican == 0)
```
...
plot(density(judge.means$lib_vote_share), 
	...
	ylim = c(0,2.5),
	ylab = "", 
	xlab = "Proportion of Cases Decided in a Feminist Direction", 
	yaxt = "n", 
	bty = "n", 
	main = "",
	col = "black", 
...
```{r regressions, results='asis', message=FALSE}
## Table 4: Core Results (WLS)

# In this code chunk we are going to run the regression models for all the
# judges, weighted by the number of cases they have had. It is useful to take a
# look at the coefficients before knitting using list(model_object_name). Use
# weighted linear regression formulas for all.

# The following was commented out in the replication code: 
    # judge.means <- subset(judge.means, child > 0)
# not sure why. Seems like they didn't need to subset a data frame for ave. judges with kids.

my.out1 <- lm(lib_vote_share ~ as.factor(girls) + as.factor(child), 
              data = judge.means, 
              weights = judge.means$no_cases)

my.out2 <- lm(lib_vote_share ~ I(girls>0) + as.factor(child), 
              data = judge.means, 
              weights = judge.means$no_cases)

# I() changes the class of an object to indicate that it should be treated ‘as
# is’.
...
my.out5 <- lm(lib_vote_share ~ as.factor(girls) + as.factor(child), 
              data = subset(judge.means, child < 5 & child > 0), 
              weights = judge.means$no_cases[which(judge.means$child > 0 & judge.means$child < 5)])
...
# Use the stargazer package to format the table. Stargazer supports presenting a
# large number models, and looks generally nice and clean. specify the style we
# want, ajps (American Journal of Pol. Sci.) and the table will be produced in
# Latex and html format. We would eventually like to translate this into a
# readable format in a knitted .rmd file, as in, use kable or another package
# that produces a nice-looking table but not in latex.

# in order to try to get the stargazer format to show in the knitted .rmd, place
# "r, results='asis'" into the r chunk title box.
...
stargazer(my.out1, 
          ...
          my.out8,
          header = FALSE,
          title = "Weighted Least Squares Results, Gender-Related Cases Only",
          style = "ajps", 
          omit.stat = c("f","ser"),
          dep.var.lables = "Voting Record in a Feminist Direction, Gender-Related Cases Only",
          ...
          digits = 2, 
          #we also want to add those nice "buckets" titles that table 4 has,
          #creating a visual distinction between models 1-4, "All Judges" who
          #have any number of children, and models 5-8, "Judges with 1-4
          #Children", showing more clearly the relationship between having some
          #kids versus...small $ judges with girls bc they have a lot of kids
          #isnt driving the regression. exclude kids>4, but the reason they
          #include judges with 4 kids on the right bucket (models 5-8) is to
          #make the results more robust and to assure their readers that they
          #aren't excluding judges with 4 kids in other regressions just because
          #of that negative coefficient that pops up in model 1.
          column.labels = c("All Judges", "Judges with 1-4 Children"),
          column.separate = c(4, 3),
          omit = c("circuit"), 
          covariate.labels = c("1 Girl",
                               "2 Girls",
                               "3 Girls",
                               "4 Girls",
                               "5 Girls",
...
Generally don't want to leave raw or unformated regression output in a document, it looks a little messy, but the final Table 4 product looks really nice!

```{r, results = 'asis', message=FALSE}
## Table 5: Core Results (Case-Level)

my.out7 <- glm(progressive.vote ~ as.factor(girls) + as.factor(child), family = binomial("logit"),
		data = subset(women.cases, 
		              child < 5 & child > 0))
summary(my.out7)

my.out8 <- glm(progressive.vote ~ I(girls > 0) + as.factor(child), family = binomial("logit"), 
		data = subset(women.cases, 
		              child < 5 & child > 0))
summary(my.out8)

my.out9 <- glm(progressive.vote ~ I(girls>0) + as.factor(child) + republican + age + I(religion == 4) + woman + I(race == 2) + I(race ==3) + as.factor(circuit) + as.factor(year), 
...
## Note: Running the ordered logit with polr, in subsetting the data, there are
## some case areas in which there are no observations. Those coefficients are
## being dropped at POLR gives an warning. Nothing to worry.
...
# caselist <- lapply(casenames, 
#                    function(x) dat12[which(dat12$casename == x),])
# names(caselist) <- casenames

## Running the bootstraps
# Note from the authors: not all of the coefficients will be sampled in any given of the 
...
# boots <- 1000
# b.star11 <- matrix(NA, 
#                    ncol =length(my.out11$result$coef),
#                    nrow = boots)
# colnames(b.star11) <- names(my.out11$result$coef)
# b.star12 <- matrix(NA, 
#                    ncol =length(my.out12$result$coef),
#                    nrow = boots)
# colnames(b.star12) <- names(my.out12$result$coef)
# set.seed(1234)
# for (b in 1:boots) {
# 	if ((b%%10) == 0) cat("boot: ", b, "\n")
# 	clust.sample <- sample(casenames,
# 	                       replace = TRUE)
# 	c.boot <- do.call(rbind,lapply(clust.sample,
# 	                               function(x) caselist[[x]]))
...
# 	data = c.boot, 
# 	family = binomial("logit"))
#     out12.star <- polr(as.factor(vote) ~ I(girls>0) + as.factor(child) + republican + age + I(religion == 4) + woman + I(race == 2) + I(race ==3) + as.factor(circuit) + as.factor(year) + as.factor(area),
#                        data = c.boot)	
...
stargazer(my.out7, 
         ...
         # my.out12, 
          style = "ajps",
          omit.stat = c("f","ser"),
          dep.var.labels = "Logit and ordered logit results, gender cases only. Outcome is whether judge in a case votes in a feminist direction (Columns 1--5) or in a conservative, moderate, or liberal direction (Column 6).  All models include fixed effects for total number of children and Columns 3--6 include circuit and year fixed effects. Column 5 additionally includes standard errors clustered at the case level",
...
my.outWOMEN <- lm(lib_vote_share ~ I(girls > 0) + as.factor(child), 
                  data = women.means, 
                  weights = women.means$no_cases)
summary(my.outWOMEN)

my.outREPMEN <- lm(lib_vote_share ~ I(girls > 0) + as.factor(child), 
                   data = subset(men.means, 
                                 republican == 1), 
                   weights = men.means$no_cases[which(men.means$republican == 1)])
...
stargazer(my.outREP, 
          ...
          style = "ajps", 
          omit.stat = c("f","ser"), 
          dep.var.labels = "Share of Votes in Feminist Direction", 
          digits = 2, covariate.labels = c("At Least 1 Girl",
                                           "2 Children", 
                                           "3 Children", 
                                           "4 Children",
                                           "Constant"), 
          title = "Weighted least squares results. Outcome is judges' proportion of feminist votes on gender-related cases. All models include fixed effects for total number of children and use weights based on the number of cases heard by each judge.", 
...
stargazer(my.out12, 
          ...
          style = "ajps", 
          omit.stat = c("f","ser"), 
          dep.var.labels = "Liberal Judge-Vote")
```
...
aa <- table(dem.means$child, 
            dem.means$girl)
bb <- table(rep.means$child, 
            rep.means$girl)

aa.1 <- prop.table(table(dem.means$child, 
                         dem.means$girl),1)
bb.1 <- prop.table(table(rep.means$child,
                         rep.means$girl),1)
...
table_9 <- cbind(aa.1, 
             bb.1)
table_9 %>% 
  kable(caption = "Distribution of gender-related cases") %>% 
  kable_styling(full_width = F)
```
...
#           omit.stat = c("f","ser"), 
#           dep.var.labels = "Liberal Judge-Vote", 
#           title = "Logit regression results, gender cases only. Outcome is whether judge in a case votes in a feminist direction. Column 1 includes judges with 0 or 1 child. Columnes 2--3 includes judges with 1 child only.")
```
...
Nice job overall. In the future you'll definitely want to make sure there isn't unformatted regression output scattered throughout the document because it's pretty distracting, but this is good first replication! Your comments are very helpful and detailed.
